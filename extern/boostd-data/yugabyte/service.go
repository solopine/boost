...
func (s *Store) GetOffsetSize(ctx context.Context, pieceCid cid.Cid, hash mh.Multihash) (*model.OffsetSize, error) {
	...
	if txPiece != nil {
		offset, size, err = s.getTxOffsetSize(ctx, hash, txPiece.Version)
		if err != nil {
			return nil, err
		}
	} else {
...
func (s *Store) getTxPieceFromIdxDb(ctx context.Context, pieceCid cid.Cid) (*txcarlib.TxPiece, error) {
	...
	txPiece.Version = txcarlib.TxCarVersion(version)
	return &txPiece, nil
}
...
func (s *Store) TxPiecesContainingMultihash(ctx context.Context, m mh.Multihash) ([]cid.Cid, error) {
	...
	return s.GetTxPiecesByVersion(ctx, txcarlib.TxCarVersion(version))
}

func (s *Store) GetTxPiecesByVersion(ctx context.Context, version txcarlib.TxCarVersion) ([]cid.Cid, error) {
	// Get all piece cids referred to by the multihash
...
func (s *Store) getTxOffsetSize(ctx context.Context, hash mh.Multihash, version txcarlib.TxCarVersion) (uint64, uint64, error) {
	var offset, size uint64
	if version == txcarlib.TxCarV1 {
...
func (s *Store) getTxOffsetSizeIter(ctx context.Context, version txcarlib.TxCarVersion) (*gocql.Iter, error) {
