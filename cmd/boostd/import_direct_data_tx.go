...
var importDirectDataTxCmd = &cli.Command{
	...
	Action: func(cctx *cli.Context) error {
		...
		txCarInfoStr = storagemarket.TxCarKeyPrefix + txCarInfoStr
		txCarInfo, err := storagemarket.ParseTxCarInfo(txCarInfoStr)
		if err != nil {
			return fmt.Errorf("txCarInfo invalid: %w", err)
		}
		...
		if startEpoch == 0 {
			startEpoch = head.Height() + (builtin.EpochsInDay * 3)
		}
