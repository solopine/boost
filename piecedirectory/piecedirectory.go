...
func NewPieceDirectory(store *bdclient.Store, pr types.PieceReader, db *sql.DB, addIndexThrottleSize int, opts ...Option) *PieceDirectory {
...
func (ps *PieceDirectory) BlockstoreGet(ctx context.Context, c cid.Cid) ([]byte, error) {
	...
	if txPiece != nil {
		...
		return txcarlib.TxBlockGet(ctx, txPiece.CarKey, offsetSize.Offset, offsetSize.Size, txPiece.Version)
	}
