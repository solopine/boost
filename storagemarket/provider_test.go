...
func NewHarness(t *testing.T, opts ...harnessOpt) *ProviderHarness {
	...
	pm := piecedirectory.NewPieceDirectory(bdclientutil.NewTestStore(pdctx), minerStub.MockPieceReader, nil, 1)
	pm.Start(pdctx)
	t.Cleanup(cancel)
...
func (h *ProviderHarness) shutdownAndCreateNewProvider(t *testing.T, opts ...harnessOpt) {
	...
	pdctx, cancel := context.WithCancel(context.Background())
	pm := piecedirectory.NewPieceDirectory(bdclientutil.NewTestStore(pdctx), h.MinerStub.MockPieceReader, nil, 1)
	pm.Start(pdctx)
	t.Cleanup(cancel)
